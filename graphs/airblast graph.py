...
def onclick(event):
    ...
    fig.suptitle("{:.0f}千吨当量低空空爆".format(Y), fontsize="x-large")
    ax4.set_title("超压杀伤效果")
    if mode == 0:
        ax5.set_title("超压抵达及正压时间（秒）")
        ax6.set_title("超压冲量")
    else:
        ax5.set_title("动压抵达及正压时间（秒）")
        ax6.set_title("动压冲量")

    for ax in lows + highs:
        ax.set_aspect("equal")
        ax.set_xlabel("投影距离（千米）")
        ax.set_ylabel("爆高（千米）")
    ...
    lbls = ("轻微破坏", "中等破坏", "严重破坏")
    legend2 = ax4.legend(nms, lbls, title="低层砖木、混凝土楼", ncol=1, loc="upper right")
    ...
    ax2.clabel(cst2, fmt=lambda x: "{:.2f} s".format(x))
    ...
    ax5.clabel(cst5, fmt=lambda x: "{:.1f} s".format(x))
    ...
    cbar2 = fig.colorbar(csd2, cax=axins2, format="%.2f", label="持续时间（秒）")
    ...
    cbar5 = fig.colorbar(csd5, cax=axins5, format="%.2f", label="持续时间（秒）")
